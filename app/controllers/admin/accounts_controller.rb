class Admin::AccountsController < Admin::AdminController
  ...
  def index
    ...
    @account_count = Account.joins(:account_products).joins(:products).where(condition).group('accounts.id').count.count
    @accounts = Account.joins(:account_products).joins(:products).where(condition).page(params[:page]).per(params[:per_page]).group('accounts.id').order('id desc');
  end
